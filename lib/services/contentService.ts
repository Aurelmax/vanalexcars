...
class ContentService {
  ...
  async getWordPressPosts(params?: {
    ...
    const queryString = searchParams.toString();
    const endpoint = queryString
      ? `${this.baseEndpoint}/wordpress/posts?${queryString}`
      : `${this.baseEndpoint}/wordpress/posts`;

    return apiClient.get<WordPressPost[]>(endpoint);
  }

  async getWordPressPostById(id: number): Promise<ApiResponse<WordPressPost>> {
    return apiClient.get<WordPressPost>(
      `${this.baseEndpoint}/wordpress/posts/${id}`
    );
  }
